...
//Process remaining objects
{
  ...
  if(_type == "Sign_Arrow_Cyan_F") then {
    	_ASL = _x getVariable ["ASL", true];
  } else {
      _ASL = _x getVariable ["ASL", false];
  };


	if (!_ASL) then {_objPos = position _x;} else {_objPos = getPosATL _x;}; //To cover some situations (inside objects with multiple roadways)
