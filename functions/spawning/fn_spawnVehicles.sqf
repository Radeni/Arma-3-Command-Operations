...
params ["_wpParams", "_minRadius", "_maxRadius", "_groupCount", "_groupName", "_groupSideParam", "_groupFactionParam", "_vehTypeParam","_unitCount", "_formationParam", "_sim"];
...
for "_a" from 1 to _groupCount do {
    ...
    if (_groupSide == civilian) then {
        _unitArray = _factionArray select 1;
    } else {
        _vehLight = _factionArray select 3;
        _vehMedium = _factionArray select 4;
		if (count _vehMedium == 0) then {_vehMedium = _vehLight};
        _vehHeavy = _factionArray select 5;
		if (count _vehHeavy == 0) then {_vehHeavy = _vehMedium};

        //Pre-select classnames based on menType parameter
        _unitArray = switch (_vehType) do {
            case "LIGHT": {_vehLight};
            case "MEDIUM": {_vehMedium};
            case "HEAVY": {_vehHeavy};
            default {selectRandom [_vehLight, _vehMedium, _vehHeavy]}; //Pre-select randomly if unspecified
        };
    };
	...
	for "_b" from 1 to _unitsAmount do {
        ...
        _unit = createVehicle [(selectRandom _unitArray),_vehPos,[],25,"NONE"];
